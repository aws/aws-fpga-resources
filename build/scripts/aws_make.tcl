  ...
  # Categories:
	...
	if {[info exist ::env(FAAS_CL_DIR)]} {
		if {[file exist $::env(FAAS_CL_DIR)/build/constraints/cl_clocks_aws.xdc] eq 0} {
			...
			file copy -force $::aws::make_faas::_nsvars::script_dir/../constraints/cl_clocks_aws.xdc $::env(FAAS_CL_DIR)/build/constraints/cl_clocks_aws.xdc
...
add_files -fileset sim_1 [ list \
 $::env(HDK_SHELL_DESIGN_DIR)/../../ip/cl_ip/cl_ip.srcs/sources_1/ip/axi_clock_converter_0/axi_clock_converter_0.xci\
 $::env(HDK_SHELL_DESIGN_DIR)/../../ip/cl_ip/cl_ip.srcs/sources_1/ip/axi_register_slice/axi_register_slice.xci\
 $::env(HDK_SHELL_DESIGN_DIR)/../../ip/cl_ip/cl_ip.srcs/sources_1/ip/cl_ddr4/cl_ddr4.xci
]
...
set_property -name {questa.simulate.vsim.more_options} -value {-sv_lib libdpi +notimingchecks} -objects [get_filesets sim_1]
...
set_property -name {ies.elaborate.ncelab.more_options} -value {+libext+.v+.sv -disable_sem2009 -timescale 1ps/1ps -notimingchecks} -objects [get_filesets sim_1]
...
set_property -name {vcs.compile.vlogan.more_options} -value {-g -ntb_opts tb_timescale=1ps/1ps -timescale=1ps/1ps -sverilog +systemverilogext+.sv +libext+.sv +libext+.v -full64 -lca -v2005 +v2k +define+VCS +lint=TFIPC-L} -objects [get_filesets sim_1]
set_property -name {vcs.elaborate.vcs.more_options} -value {-ignore initializer_driver_checks} -objects [get_filesets sim_1]
set_property -name {vcs.simulate.vcs.more_options} -value {-sv_lib libdpi +notimingcheck} -objects [get_filesets sim_1]
...
#	return [make_faas -bypass_drcs -partial {*}$args]
	...
	set_property USED_IN {synthesis synthesis out_of_context implementation} [get_files */cl_clocks_aws.xdc]
        ...
        if {[current_project -quiet] ne ""} {
		if {[get_files *pnr_user.xdc -quiet] == ""} {
		...
		set ::env(PNR_USER) [get_files *pnr_user.xdc]
		}
...
add_files -fileset sim_1 [ list \
  $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/axi_bfm_defines.svh \
  $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/axi_mem_model.sv  \
  $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/axi4_slave_bfm.sv  \
  $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/sh_ddr.sv
]

set_property used_in_synthesis false [get_files $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/sh_ddr.sv]
set_property used_in_implementation false [get_files $::env(HDK_SHELL_DESIGN_DIR)/sh_ddr/sim/sh_ddr.sv]
