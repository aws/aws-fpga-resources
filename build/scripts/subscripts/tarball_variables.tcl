...
set HDK_VERSION_DIR "shell_v04052421"
set debugMode 1
...
if { [info exists ::env(HDK_SHELL_DIR)] } {
	...
	set HDK_SHELL_DIR [file join $HDK_COMMON_DIR shell_stable]
...
if { [info exists ::env(AWS_FPGA_REPO_DIR)] } {
        set AWS_FPGA_REPO_DIR $::env(AWS_FPGA_REPO_DIR)
} else {
	set AWS_FPGA_REPO_DIR [file join $HDK_DIR ../]
	set ::env(AWS_FPGA_REPO_DIR) $AWS_FPGA_REPO_DIR
}

set hdk_version [tclapp::xilinx::faasutils::make_faas::findLineInFile  [file join $AWS_FPGA_REPO_DIR release_version.txt] "RELEASE_VERSION"]
...
set shell_version [tclapp::xilinx::faasutils::make_faas::findLineInFile  [file join $HDK_SHELL_DIR shell_version.txt] "${shell}"]
...
set clock_main_a0 $::env(clock_main_a0)
