...
#			send_msg_id "opt_design_pre 0-1" ERROR "FAAS_CL_DIR environment varaiable not set, please run the proc 'aws::make_faas_setup' at the Vivado TCL command prompt"
	...
	set AWS_RTL_XDC_EXISTS [get_files */cl_timing_aws.xdc]

	set TIMING_USR_LOC [get_files */cl_timing_user.xdc]
	...
	add_files $::env(HDK_SHELL_DIR)/build/checkpoints/from_aws/cl_bb_routed.small_shell.dcp
	} elseif {$shell == "xdma_shell" } {
	add_files $::env(HDK_SHELL_DIR)/build/checkpoints/from_aws/cl_bb_routed.xdma_shell.dcp
	} else {
	send_msg_id "opt_design_pre 0-1" ERROR "shell variable is not found, set shell variable in init.tcl or Vivado_init.tcl to small_shell or xdma_shell"
	}
	...
	set_property SCOPED_TO_CELLS {WRAPPER/CL} [get_files $FAAS_CL_DIR/build/checkpoints/CL.post_synth_inline.dcp]
	} else {
	add_files $FAAS_CL_DIR/build/checkpoints/CL.post_synth.dcp
	set_property SCOPED_TO_CELLS {WRAPPER/CL} [get_files $FAAS_CL_DIR/build/checkpoints/CL.post_synth.dcp]
	}

	read_xdc $::env(HDK_SHELL_DIR)/build/constraints/cl_pins.xdc
	set_property PROCESSING_ORDER LATE [get_files $::env(HDK_SHELL_DIR)/build/constraints/cl_pins.xdc]

	#read_xdc $::env(HDK_SHELL_DIR)/build/constraints/xdma_shell_level_1_fp_cl.xdc
	...
	if {$TIMING_USR_LOC != ""} {
		read_xdc ${TIMING_USR_LOC}
		set_property PROCESSING_ORDER LATE [get_files $TIMING_USR_LOC]
		set_property USED_IN {implementation} [get_files $TIMING_USR_LOC]
		}
	...
	#link_design -top $top -part [get_parts -of_objects [current_project]] -reconfig_partitions {WRAPPER_INST/SH WRAPPER_INST/CL}
        link_design -mode default -reconfig_partitions {WRAPPER/CL} -top $top

	source ${FAAS_CL_DIR}/build/constraints/aws_gen_clk_constraints.tcl

	#For RTL flow
	if {$AWS_RTL_XDC_EXISTS != ""} {
        set CL_NAME [get_property REF_NAME [get_cells WRAPPER/CL]]
        read_xdc -ref ${CL_NAME} $::env(HDK_SHELL_DESIGN_DIR)/../../shell_stable/build/constraints/cl_timing_aws.xdc
	}

	#Check if sh_ddr is not using DDR4 IP, if so change the diff pair IOSTANDARD to avoid error
        set no_ddr4_ip  [get_cells -hierarchical  -filter {NAME =~ */genblk1.IS_DDR_PRESENT.DDR4_0} -quiet]

	if {$no_ddr4_ip == ""} {
          set_property IOSTANDARD  SSTL12_DCI [get_ports {M_CLK_DP[0]}]
          set_property IOSTANDARD  SSTL12_DCI [get_ports {M_CLK_DN[0]}]
        } else {
          #Set 3 clock region pblock for MIG IP
          set_property MIG_FLOORPLAN_MODE FULL [get_cells -hierarchical  -filter {NAME =~ */genblk1.IS_DDR_PRESENT.DDR4_0} ]
          set_property IOSTANDARD  SSTL12_DCI [get_ports {M_CLK_DP[0]}]
          set_property IOSTANDARD  SSTL12_DCI [get_ports {M_CLK_DN[0]}]
        ...
        #Allowing for clk_wiz instances connected in the CL
        set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets WRAPPER/IO_SHIM/clk_hbm_ref]
