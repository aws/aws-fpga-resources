...
proc init_gui { IPINST } {
  ...
  set HBM_PRESENT       [ipgui::add_param $IPINST -name "HBM_PRESENT"         -parent ${interfaces_group} -widget checkBox     ]
  ...
  set CLOCK_HBM_RECIPE      [ipgui::add_param $IPINST -name "CLOCK_HBM_RECIPE"        -parent ${clock_group} -widget comboBox  ]
    ipgui::add_indent $IPINST -parent ${clock_group}
  set CLOCK_HBM_FREQ       [ipgui::add_param $IPINST -name "CLOCK_HBM_FREQ"         -parent ${clock_group}                    ]
    ipgui::add_indent $IPINST -parent ${clock_group}
  ...
  set_property tooltip "DDR memory in CL" $DDR_A_PRESENT       
  set_property tooltip "Enable Monitor Port and clocks for HBM in CL" $HBM_PRESENT       
  ...
  set_property tooltip "Enables irq_req, irq_ack, status_vdip, status_vled, flr_assert, flr_done, glcount" $AUX_PRESENT       
  ...
  set_property tooltip "Pipelining of DDR statistics signals between DDR and Shell" $NUM_STAGES_STATS  
  set_property tooltip "Select frequency of clk_main_a0" $CLOCK_A_RECIPE     
  set_property tooltip "Extra Clocks in Group A are edge-aligned and generated from the same PLL" $NUM_A_CLOCKS       
...
proc update_gui_for_PARAM_VALUE.HBM_PRESENT { IPINST PARAM_VALUE.HBM_PRESENT } {
  set HBM_PRESENT [get_property value ${PARAM_VALUE.HBM_PRESENT}]
  if {$HBM_PRESENT>0} {
    set_property visible true  [ ipgui::get_guiparamspec CLOCK_HBM_RECIPE -of $IPINST ]
    set_property visible true  [ ipgui::get_guiparamspec CLOCK_HBM_FREQ -of $IPINST ]
  } else {
    set_property visible false [ ipgui::get_guiparamspec CLOCK_HBM_RECIPE -of $IPINST ]
    set_property visible false [ ipgui::get_guiparamspec CLOCK_HBM_FREQ -of $IPINST ]
  }
}
...
proc update_PARAM_VALUE.CLOCK_HBM_RECIPE { PARAM_VALUE.CLOCK_HBM_RECIPE PARAM_VALUE.HBM_PRESENT } {
  set HBM_PRESENT [get_property value ${PARAM_VALUE.HBM_PRESENT}]
  if {$HBM_PRESENT==0} {
    set_property range 0,0 ${PARAM_VALUE.CLOCK_HBM_RECIPE}
    set_property enabled false ${PARAM_VALUE.CLOCK_HBM_RECIPE}
  } else {
    set_property range 0,4 ${PARAM_VALUE.CLOCK_HBM_RECIPE}
    set_property enabled true ${PARAM_VALUE.CLOCK_HBM_RECIPE}
  }
}

proc update_PARAM_VALUE.CLOCK_A0_FREQ { PARAM_VALUE.CLOCK_A0_FREQ PARAM_VALUE.CLOCK_A_RECIPE } {
  set clock_freq [list 250000000 250000000 250000000]
...
proc update_PARAM_VALUE.CLOCK_C1_FREQ { PARAM_VALUE.CLOCK_C1_FREQ PARAM_VALUE.CLOCK_C_RECIPE } {
  set clock_freq [list 400000000 200000000 100000000 266666667]
  set CLOCK_C_RECIPE [get_property value ${PARAM_VALUE.CLOCK_C_RECIPE}]
...
proc update_PARAM_VALUE.CLOCK_HBM_FREQ { PARAM_VALUE.CLOCK_HBM_FREQ PARAM_VALUE.CLOCK_HBM_RECIPE } {
  set clock_freq [list 250000000 125000000 450000000 300000000 400000000]
  set CLOCK_HBM_RECIPE [get_property value ${PARAM_VALUE.CLOCK_HBM_RECIPE}]
  set_property value [lindex $clock_freq $CLOCK_HBM_RECIPE] ${PARAM_VALUE.CLOCK_HBM_FREQ}
  set_property enabled false ${PARAM_VALUE.CLOCK_HBM_FREQ}
}
...
proc update_MODELPARAM_VALUE.C_CLOCK_HBM_PERIOD { MODELPARAM_VALUE.C_CLOCK_HBM_PERIOD PARAM_VALUE.CLOCK_HBM_FREQ } {
  set freq [get_property value ${PARAM_VALUE.CLOCK_HBM_FREQ}]
  set period [expr 1000000000.0 / $freq]
	set_property value $period ${MODELPARAM_VALUE.C_CLOCK_HBM_PERIOD}
}
...
proc update_MODELPARAM_VALUE.C_CLOCK_HBM_RECIPE { MODELPARAM_VALUE.C_CLOCK_HBM_RECIPE PARAM_VALUE.CLOCK_HBM_RECIPE } {
	set_property value [get_property value ${PARAM_VALUE.CLOCK_HBM_RECIPE}] ${MODELPARAM_VALUE.C_CLOCK_HBM_RECIPE}
}
...
proc update_PARAM_VALUE.HBM_PRESENT { PARAM_VALUE.HBM_PRESENT PARAM_VALUE.MODE } {
	# Procedure called to update HBM_PRESENT when any of the dependent parameters in the arguments change

    set_property range 0,1 ${PARAM_VALUE.HBM_PRESENT}
    set_property enabled true ${PARAM_VALUE.HBM_PRESENT}
...
proc update_MODELPARAM_VALUE.C_HBM_PRESENT { MODELPARAM_VALUE.C_HBM_PRESENT PARAM_VALUE.HBM_PRESENT } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.HBM_PRESENT}] ${MODELPARAM_VALUE.C_HBM_PRESENT}
}
